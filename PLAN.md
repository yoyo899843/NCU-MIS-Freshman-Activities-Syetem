...
原本規劃是走 Socket.IO 的 `team:<teamId>` room（只有同隊看得到彼此），實際做出來的版本改成：**大地圖直接顯示所有登入玩家的位置，不分陣營也不分隊伍**，改用固定 2 秒週期的 REST 輪詢（`POST/GET /api/locations`，`playerAuth` 保護），不用 Socket.IO 推播（`sockets/index.js` 保留註解說明這段改動）。座標刻意不落地到 DB，只存在伺服器記憶體（`src/playerLocations.js`），process 重啟就整批清空，可接受——下一輪玩家端上傳會自動補回來。
